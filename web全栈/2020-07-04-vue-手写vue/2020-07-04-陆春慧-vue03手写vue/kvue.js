...
function proxy(vm) {
  ...
  // 代理methods
  Object.keys(vm.$options.methods).forEach(key => {
    Object.defineProperty(vm, key, {
      get() {
        return vm.$options.methods[key]
      },
      set(v) {
        console.error('can not set method')
      }
    })
  })
  ...
  compileElement(node) {
    ...
    Array.from(nodeAttrs).forEach(attr => {
      ...
      if (attrName.startsWith('@') || attrName.startsWith('v-on:')) {
        if (this.$vm[exp] && this.$vm[exp] instanceof Function) {
          let event;
          if (attrName.startsWith('@')) {
            event = attrName.substring(1);
          } else {
            event = attrName.substring(5);
          }
          node.addEventListener(event, (e) => {
            this.$vm[exp].call(this.$vm, e);
          })
        }
